...
- **NEVER pick arbitrary spacing values.** All margins, paddings, and gaps must follow a consistent spacing scale throughout the entire project.
- **Spacing scale** (base 8px): 4, 8, 12, 16, 24, 32, 48, 64, 96.
- **Equal elements = equal spacing.** If two elements have the same hierarchical relationship (e.g., subtitle → button, title → subtitle), their spacing must be identical unless there is a clear design reason.
- **Before adding any spacing value**, check what spacing is already used for similar elements on the page and match it.
- **Desktop side padding**: 70px — project standard for all sections.
